...
class DailyReportBuilder:
    ...
    def build_active_period_banner(self):
        ...
        busiest_time = timedelta(
            hours=self.analyzer.analyze_busiest_time_in(self.day_start, self.day_end)
        )
        active_period_start = (self.day_start + busiest_time).strftime("%H:%M")
        active_period_end = (
            self.day_start + busiest_time + timedelta(hours=1)
        ).strftime("%H:%M")
    ...
    def build_active_user_count_banner(self):
        if self.report.big_banners is None:
            self.report.big_banners = []
        active_user_count = str(
            self.analyzer.analyze_daily_active_user(self.day_start, self.day_end)
        )
    ...
    def build_week_bar_plot(self):
        ...
        final_bar_containers = self._calculate_bar_percentage(raw_bar_containers)
    ...
    def build_day_bar_plot(self):
        ...
        yesterday_message_count = self.analyzer.get_message_count_between(
            self.day_start - timedelta(days=1), self.day_end - timedelta(days=1)
        )
        trend_percentage = self.analyzer.calculate_trend_percentage(
        ...
        trend_icon = self.analyzer.get_trend_icon(trend_percentage, 5)
        plot_head = BarPlotHead(
            f"{self.analyzer.get_message_count_between(self.day_start, self.day_end)}条",
            "今日消息总数",
            f"{trend_icon} 相较昨天浮动 {trend_percentage}%",
        )

        # bar plot body
        raw_bar_containers = self._build_day_bar_segments(self.day_start)
        final_bar_containers = self._calculate_bar_percentage(raw_bar_containers)
        plot_body = BarPlotBody(
            final_bar_containers,
            [BarPlotLegend("text"), BarPlotLegend("image"), BarPlotLegend("other")],
