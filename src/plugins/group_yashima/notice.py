...
def do_schedule(func: Callable, job_id: str, args: Optional[list] = None) -> None:
    """将任务添加到scheduler中"""
    connection_cooldown_time: float = 30.0
    running_time = datetime.now() + timedelta(seconds=connection_cooldown_time)
    if scheduler.get_job(job_id):
        scheduler.remove_job(job_id)

    logger.info(f"register scheduler for {job_id})")
...
def build_notice_content(platform_name: str, bot_id: str) -> str:
    """生成掉线提醒消息"""
    global connect_start_time
...
def do_send_mail(title: str, content: str) -> None:
    """发送邮件通知"""
    mail_user: str = get_config().notice.user
    mail_host: str = get_config().notice.host
    mail_port: int = get_config().notice.port
    mail_key: str = get_config().notice.key
