...
from ..types import Category, RawPost, SubUnit, Tag, Target
from ..utils import ProcessContext, Site
...
async def catch_network_error(
    ...
    try:
        ...
        logger.warning(f"unknown error occured: {err}")
...
class Platform(metaclass=PlatformABCMeta, base=True):
    ...
    async def filter_user_custom(
        ...
        for raw_post in raw_post_list:
            if self.categories:
                cat = int(self.get_category(raw_post))
    ...
    async def dispatch_user_post(
        ...
        res: list[tuple[int, list[Post]]] = []
        for user, cats, required_tags in sub_unit.channel_sub_infos:
    ...
    async def _handle_new_post(
        ...
        if not new_posts:
            ...
            for post in new_posts:
                logger.info(
                    f"fetch new post from {self.platform_name} {sub_unit.sub_target if self.has_target else '-'}: {self.get_id(post)}"
                )
        res = await self.dispatch_user_post(new_posts, sub_unit)
        self.parse_cache = {}
        return res

    async def fetch_new_post(self, sub_unit: SubUnit) -> list[tuple[int, list[Post]]]:
