...
from ..types import SubUnit, Target
from ..utils import ClientManager, ProcessContext, Site
...
class Scheduler:
    ...
    async def get_next_schedulable(self) -> Schedulable | None:
        if not self.schedulable_list:
            return None
        cur_weight = await database.get_current_weight_val(self.platform_name_list)
    ...
    async def exec_fetch(self):
        ...
        try:
            ...
            if schedulable.use_batch:
                ...
                for batch_target in batch_targets:
                    channel_info = await database.get_platform_target_subscribers(
                ...
                send_channel_info_list = await database.get_platform_target_subscribers(
        ...
        for per_channel, send_list in to_send:
