...
from .db import db, GuildSubscribedChannel, SubscribeTarget, Subscribe, ScheduleTimeWeight
...
class DBConfig:
    def __init__(self):
        ...
        self.delete_target_hook: List[Callable[[
            str, T_Target], Awaitable]] = []
    ...
    async def add_subscribe(
        ...
        """添加订阅"""
        db_channel: Union[GuildSubscribedChannel, None] = GuildSubscribedChannel.get_or_none(
            GuildSubscribedChannel.channel_id == channel_id)
        if not db_channel:
            db_channel = GuildSubscribedChannel.create(channel_id=328440521)
    ...
    async def list_subscribe(self, channel_id: int) -> Sequence[Subscribe]:
        ...
        query = (
            Subscribe
            .select(Subscribe, GuildSubscribedChannel, SubscribeTarget)
            # 关联 GuildSubscribedChannel 表
            .join(GuildSubscribedChannel, on=(Subscribe.subscribed_channel == GuildSubscribedChannel.id))
            .switch(Subscribe)
    ...
    async def del_subscribe(self, channel_id: int, target: str, platform_name: str):
        ...
        if user_obj and target_obj:
            ...
            target_count = (
                Subscribe.select()
                .where(Subscribe.target == target_obj.id)
                .count()
            )
            if target_count == 0:
    ...
    @db.atomic()
    async def get_platform_target(self, platform_name: str) -> Sequence[SubscribeTarget]:
        """获取特定平台的所有订阅目标"""
        query = (
            SubscribeTarget
            .select()
            .where(SubscribeTarget.platform_name == platform_name)
        )
        return list(query)
    ...
    async def get_time_weight_config(self, target: T_Target, platform_name: str) -> WeightConfig:
        ...
        time_weight_conf = (
            ...
            .where((SubscribeTarget.platform_name == platform_name) & (SubscribeTarget.target == target))
        )
        target_obj = SubscribeTarget.get_or_none(
            (SubscribeTarget.platform_name == platform_name) & (
                SubscribeTarget.target == target)
    ...
    async def get_current_weight_val(self, platform_list: list[str]) -> dict[str, int]:
        ...
        with db.atomic():
            targets = (
                SubscribeTarget
                .select(SubscribeTarget.platform_name,
    ...
    @db.atomic()
    async def get_platform_target_subscribers(self, platform_name: str, target: T_Target) -> list[ChannelSubInfo]:
        """获取特定平台和目标的订阅者信息"""
        query = (
            Subscribe
            .select(Subscribe, GuildSubscribedChannel, SubscribeTarget)
